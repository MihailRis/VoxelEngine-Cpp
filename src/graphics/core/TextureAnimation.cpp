...
void TextureAnimator::addAnimations(
    const std::vector<TextureAnimation>& animations
) {
...
void TextureAnimator::update(float delta) {
    ...
    for (auto& elem : animations) {
        ...
        if (frameNum != elem.currentFrame) {
            ...
            float srcPosY = elem.srcTexture->getHeight() - frame.size.y -
                            frame.srcPos.y;  // vertical flip
